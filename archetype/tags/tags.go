package tags

import "github.com/yohamta/donburi"

var (
	ArmorTag      = donburi.NewTag("armor")
	CameraTag     = donburi.NewTag("camera")
	CoinTag       = donburi.NewTag("coin")
	ConsumableTag = donburi.NewTag("consumable")
	ItemTag       = donburi.NewTag("item")
	LevelTag      = donburi.NewTag("level")
	MonsterTag    = donburi.NewTag("monster")
	PickupTag     = donburi.NewTag("pickup")
	PlayerTag     = donburi.NewTag("player")
	UITag         = donburi.NewTag("ui")
	ValuableTag   = donburi.NewTag("valuable")
	WeaponTag     = donburi.NewTag("weapon")
)
